...
public class PhoneController {
	 ...
	 * 해당 핸드폰들의 평균 가격을 구해서 반환해주는 기능의 메소드
	 * 
	 * @param phones 판매중인 핸드폰 목록의 배열객체
	 * @return 핸드폰들의 평균 가격
	 */
	...
	public int searchPhonePrice(Phone[] phones, String buy) {
		...
		*/
		for(Phone p : phones) {
			if( (p.getName() + p.getSeries()).equals(buy));
			return p.getPrice();
		}
	...
	/**
	 * 판매중인 핸드폰 목록과
	 * 검색하고자 하는 핸드폰의 정보(키워드값)를 전달받아
	 * 검색된 핸드폰의 가격을 반환해주는 기능의 메소드
	 * 단, 키워드 검색으로 구현
	 *  ㄴ 일치하는지가 아닌 포함하고 있는지 비교하기
	 *  
	 *  이때 검색결과가 여러개일 수 있으니
	...
	public int[] searchKeywordPhonePrice(Phone[] phones, String buy) {
		...
		*/
		int i = 0;
		for(Phone p : phones) {
			if((p.getName() + p.getSeries()).contains(buy)) {
				prices[i] = p.getPrice();
