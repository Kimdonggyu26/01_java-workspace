...
public class Array {
	...
	public void method1() {
		...
		*/
		 ...
		 * 어떤 "자료형"의 값들을 "몇개"에 담을건지 고려해서 만들어야됨
		 * 
		 * 1. 배열 선언
		 * 
		 * 		[표현법] 자료형 배열명[];
		 ...
		 *
		 * 2. 배열 생성(할당)
		 *    몇개의 값을 담을 건지 그 크기만큼의 배열을 만들어야됨
		 *    
		 *   	[표현법] 배열명 = new 자료형[배열의 크기]
		 *   
		 * 3. 배열 선언과 동시에 생성 (1 + 2)
		 * 
		...
		System.out.println("전체 값 : " + arr);	// 배열의자료형 @ 배열의주소값이 출력됨
	...
	public void method2() {
		...
		/*
		 * 실제 데이터 값을 곧바로 담을 수 있는 변수를 일반 변수라고 함
		 * 주소값을 담고 있는 변수는 참조 변수(레퍼런스 변수)라고 함
		 * 
		 * 기본자료형 (boolean, char, byte, short, int, long, float, double)
		 * == 일반 변수 == 실제 데이터 값을 바로 담기 가능
		 * 
		 * 그 외 자료형(int[], double[], char[], String, Scanner, Variable, ...)으로
		 * == 참조 변수 == 주소값을 담는 변수
		 */
	...
	public void method4() {
		...
		arr[4] = 5;
		*/
		for(int i = 0; i < arr.length/*(배열의 크기)*/; i++) {
			arr[i] = i + 1;
		}
		...
		System.out.println(arr); // 자료형@주소값의 16진수형태
	...
	public void method5() {
		...
		// NullPointerException : null 가지고 특정인덱스에 접근하거나 메소드 호출하거나 할때 발생되는 오류
		// 				해결방법 : 객체생성을 해야됨
	...
	public void method6() {
		...
		*/
		...
		System.out.println("변경 후의 arr의 길이(크기) : " + arr.length);
		
		/*
		 * 연결이 끊겨진 기존 배열은 Heap 영역에 둥둥 떠다님 => 불필요한 존재가 됨 => 지워주는게 좋음
		 * -> 일정 시간이 지나면 "가비지컬렉터(GC)"가 알아서 지워줌 (자바의 특징)
		 *    자바에서의 "자동메모리관리" 특징 == 개발자는 코드 작성에만 신경쓰면 됨
		 * 
		 */
	...
	public void method7() {
		...
		//	  매번 1~100 사이의 랜덤값을 발생시켜서 대입
		
		// 3. 반복문을 활용해서 해당 배열의 모든 인덱스에 담겨있는 값 출력
		// 	  단, 인덱스에 담겨있는 값이 홀수인 것만 출력
		// 	   arr[x] : xx 형식으로 출력
		
		int[] arr = new int[10];
		
		int sum = 0; // 짝수 값들의 총 합계를 담을 변수
		int count = 0;	// 짝수 값들의 갯수를 담을 변수
		for(int i = 0; i < arr.length; i++) {
	...
	public void method8() {
		...
		// 2. 반복문을 활용해서 매번 과일명을 입력 받아서
		// 사용자가 입력한 과일명값이 exit일 경우 반복이 종료
		// 각 인덱스 자리에 대입
