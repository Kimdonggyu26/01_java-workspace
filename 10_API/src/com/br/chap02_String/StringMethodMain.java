...
public class StringMethodMain {
	...
	public static void main(String[] args) {
		...
		// 문자열.concat(문자열) : 기존 문자열에 전달 된 문자열을 덧붙여서 새로운 문자열로 반환
		String str2 = str1.concat("!!!");
		System.out.println(str2);
		
		// 문자열.equals(문자열) 		   : 두 문자열이 일치하는지 비교 결과값 반환
		// 문자열.equalsIgnoreCase(문자열) : 두 문자열이 일치하는지 비교 결과값 반환(단, 대소문자 무시)
		...
		// 문자열.contains(문자열) : 기준이 되는 문자열에 전달된 문자열이 포함되어 있을 경우 true반환
		System.out.println(str1.contains("llo"));
		
		// 문자열.substring(시작인덱스)			  : 문자열로부터 시작인덱스에서부터 끝까지 추출해서 반환
		// 문자열.substring(시작인덱스, 끝인덱스) : 문자열로부터 시작인덱스에서부터 끝 인덱스 이전까지 추출해서 반환
		System.out.println(str1.substring(6));
		System.out.println(str1.substring(0,4));
		
		// 문자열.indexOf(탐색할 문자열) : 문자열로부터 전달된 문자열의 위치(시작인덱스)
		...
		// 문자열.replace(old, new) : 문자열로부터 old문자열 찾아서 new문자열로 치환
