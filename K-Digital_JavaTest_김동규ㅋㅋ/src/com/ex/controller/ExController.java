...
public class ExController {
	...
	public void insertProduct(String name, String brand, String price) {
		...
		list.add(new Product(name, brand, prices));
	...
	public List<Product> searchProduct(String nameKeyword){
		...
		List<Product> searchNameKeyword = new ArrayList<>();
		...
		for(int i = 0; i < list.size(); i++) {
			if(list.get(i).getName().contains(nameKeyword)) {
				searchNameKeyword.add(list.get(i));
			}
		}
	
		// 해당 검색결과가 담긴 List 반환 
		
		return searchNameKeyword; // 우선 이렇게 써놓겠음 (수정해야됨)
	}
